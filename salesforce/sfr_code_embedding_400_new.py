...
class SfrCodeEmbedding400:
    ...
    def create_embedding(self, inputs: Union[str, List[str]], use_cache: bool = True) -> torch.Tensor:
        ...
        # Compute dynamic max length based on inputs to reduce unnecessary padding
        ...
        with torch.cuda.amp.autocast():  # Enable automatic mixed precision
            tokens = self.tokenizer(
                inputs,
                return_tensors="pt",
                padding="max_length" if len(inputs) > 1 else False,
                truncation=True,
    ...
    def create_embedding_with_streams(self, inputs: List[str], num_streams: int = 2) -> torch.Tensor:
        ...
        if not torch.cuda.is_available() or len(inputs) < num_streams:
            return self.create_embedding(inputs)

        # Split inputs into chunks for each stream
        chunk_size = len(inputs) // num_streams
        if chunk_size == 0:
            chunk_size = 1
    ...
    def process_batches(self, code_snippets: List[str], batch_size: int = 64) -> List[torch.Tensor]:
        ...
        for i in range(num_batches):
            ...
            # Use streams for larger batches on CUDA
    ...
    def dynamic_batching(self, code_snippets: List[str]) -> List[torch.Tensor]:
        ...
        results = []
        batch_sizes = {'short': 128, 'medium': 64, 'long': 32}
...
def benchmark_configurations() -> Dict[str, float]:
    ...
    configs = [
        ...
        {"batch_size": 128, "max_workers": 1, "enable_cache": True},
        {"batch_size": 64, "max_workers": 2, "enable_cache": True},
    ]
...
def run_benchmark(
    ...
    large_snippets = [code_snippets[0] * 10] * (max_codes // 10)  # Larger texts
    ...
    # Benchmark with regular size inputs
    print("\nRunning benchmark with regular inputs...")
    ...
    regular_qps = max_codes / total_time
    ...
    print("\nRunning benchmark with small inputs...")
    start_time = time.time()
    _ = sfr_embedder.process_in_parallel(small_snippets, batch_size=batch_size * 2, max_workers=max_workers)
    if device == "cuda":
        torch.cuda.synchronize()
    end_time = time.time()

    small_total_time = end_time - start_time
    small_qps = max_codes / small_total_time
    ...
    print("\nRunning benchmark with large inputs...")
    start_time = time.time()
    _ = sfr_embedder.process_in_parallel(large_snippets, batch_size=batch_size // 2, max_workers=max_workers)
    if device == "cuda":
        torch.cuda.synchronize()
    end_time = time.time()

    large_total_time = end_time - start_time
    large_qps = (max_codes // 10) / large_total_time

    # Print results
    print(f"\nBenchmark Results:")
    print(f"Regular inputs: {regular_qps:.2f} QPS (avg time: {total_time * 1000 / max_codes:.2f} ms)")
    print(f"Small inputs: {small_qps:.2f} QPS (avg time: {small_total_time * 1000 / max_codes:.2f} ms)")
    print(f"Large inputs: {large_qps:.2f} QPS (avg time: {large_total_time * 1000 / (max_codes // 10):.2f} ms)")
...
if __name__ == "__main__":
    ...
    if benchmark_mode == "configurations":
        ...
        results = run_benchmark(
            batch_size=64,  # For g5.xlarge, try 64 for balanced performance
